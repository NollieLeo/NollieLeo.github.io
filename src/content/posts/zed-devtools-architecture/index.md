...
*   **向过去跳转（逆向）**：依次执行 `executeReverseNetworkPatch` 进行精准的反向剥离运算。这里的反向并不是简单的“快照覆盖”，而是**绝对严谨的数学逆运算**。比如如果正向的 Patch 是 `Array.insert(2, 'a')`，那么逆运算会自动解算为 `Array.delete(2)`；如果是 `Map.set('color', 'red')`，底层引擎会从历史日志中提取旧值，将其解算为 `Map.set('color', 'blue')`。这保证了即便是回退操作，CRDT 的冲突解决时钟（Logical Clock）也依然是向前走且合法递增的。
