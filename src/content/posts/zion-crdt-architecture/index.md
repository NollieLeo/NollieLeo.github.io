...
description: "深度剖析大型无代码平台 Zion 是如何从极其消耗内存的状态化 Diff Server，重构演进为基于 json-joy 的无状态 CRDT 实时协同架构。揭秘在缺乏官方库支持的情况下，如何硬核手写基于差异的反向补丁 (Undo/Redo) 引擎与业务级冲突消解防火墙。"
tags: ["CRDT", "架构设计", "实时协同", "json-joy", "无代码"]
category: "工程化与架构"
draft: false
---

## 1. 背景与痛点：传统 Diff 架构的“阿喀琉斯之踵”

在无代码编辑器（如 Zion）中，多人实时协同编辑是一项基础且至关重要的能力。在重构之前，我们依赖于一个传统的、基于 Java 编写的 **状态化 (Stateful) Diff Server**。

这个旧架构在初期支撑了业务发展，但随着平台内项目数量的激增和画布组件树的无限膨胀，它很快暴露出几个致命的缺陷：

1. **服务端内存爆炸 (OOM)**：为了能够比对和计算两个客户端传来的 Diff，Java 服务端必须在内存中全量缓存所有活跃项目的 `AppSchema`（JSON 树）。项目一多，内存消耗极大，频繁导致 OOM 崩溃，且完全无法通过简单的横向扩容（Scale-out）来解决。
2. **中间态缓存失效**：由于缓存数量有限，一旦项目被挤出缓存，重新加载和计算 Diff 需要极长的时间。
3. **并发同步冲突**：在高频的多人协同编辑场景下，极度依赖 Server 端串行处理并推送同步更新。一旦推送延迟，客户端经常会弹出版本冲突（`old_value_mismatch`），甚至导致用户丢失大批量的编辑数据，无法保存。

为了彻底解决“服务端性能瓶颈”和“客户端合并冲突”这两座大山，我们决定引入 **CRDT (无冲突复制数据类型)**，将核心计算下放，开启一场颠覆性的架构重构。
...
在具体的库选型上，目前业界最火的是 `Yjs` 和 `Automerge`。但经过深思熟虑，我们最终选择了 **`json-joy/lib/json-crdt`**，原因非常现实且硬核：

* **完美契合底层数据结构**：Zion 编辑器的底层配置 (`AppSchema`) 是极其复杂的 JSON 树。`json-joy` 的 CRDT 节点类型原生就抽象成了 `con` (常量)、`val` (值引用)、`obj` (对象)、`arr` (数组) 等结构，这种原汁原味的 JSON 语义，极大降低了我们从旧 Schema 迁移的成本。
...
在全新的架构下，后端的角色从“计算中心”退化成了一个**无情的“补丁路由器 (Patch Router)”和“持久化存储器”**：
...
### 挑战一：填坑 `json-joy`，硬核手写 Undo/Redo 引擎
...
既然官方不支持，我们就自己造！我们扩展了原生的操作，设计了一个包含了极其详尽上下文的自定义 `Diff` 接口：
...
function reverseDiffItem(item: DiffItem): DiffItem {
  switch (item.operation) {
    ...
    // ... move 与 copy 的极其复杂的逆向拓扑运算
  }
}
```
**高光时刻**：在真正执行反转应用之前，为了防止别人刚刚把这个容器删了，你现在却要“撤销修改里面按钮的颜色”引发底层报错，我们还加入了一个 **沙盒模拟校验 (Dry-run)**。
底层调用了 `executeCrdtNodeUpdateOptimized`，在当前被别人改得“面目全非”的 CRDT 树上**虚拟执行**一次反向操作，如果抛出校验异常，说明该历史操作在当前时空已经不合法，系统会直接丢弃这个 Undo 动作；如果校验通过，逆向 Diff 最终会被转化为正常的 CRDT Patch 广播出去，完美攻克了多人时序下的时间漫游难题。
...
**解法：合并后的 `ztype` 业务防火墙**
我们在每次接收并应用远端 Patch 后，强制接入了底层业务类型校验器（基于 `ztype`），像一道防火墙一样死死守住渲染层：
...
**痛点**：面对有着数百万行遗留代码的巨型编辑器，我们不可能停下业务发版，花半年时间把所有依赖 `MobX` 状态响应式的组件全部重写为订阅 `json-joy`。
...
sequenceDiagram
    ...
    CrdtHelper->>CRDT: 2. applyLocalCrdtDiff (校验后打入真神)
    ...
    Server->>CrdtHelper: A. 收到别人发来的巨型 Patch
    CrdtHelper->>CRDT: B. 优先静默合并到 CRDT 模型
    CRDT-->>CrdtHelper: C. generateDiff: 深度对比新旧 Schema
    CrdtHelper->>UI: D. 仅将极细粒度 Diff 派发给 MobX
```

1. **Single Source of Truth**：`jsonJoyModel` 成为了真正的幕后数据主宰，所有本地操作、网络同步、冲突消解，全都在它内部原子化完成。
...
为了实现业务与底层协同引擎的解耦，我们抽离出了一个独立的 npm 包 `@functorz/crdt-helper`。它扮演了“翻译官”和“协调者”的角色：
...
const result = Crdt.executeCrdtNodeUpdateAndTransformDiff(
  [diff], // 业务 Diff
  schemaStore.jsonJoyModel, // 幕后真神：CRDT 实例
  coreStore, // 旧的 Schema 状态
  updateMobxStore, // 极其关键的回调：用于同步 MobX
);
```

**第三步：`crdt-helper` 的黑盒魔法 (`NodeUpdate/index.ts`)**
...
```
至此，UI 瞬间变成了红色。同时，`crdt-helper` 返回的 `patchBase64` 会被推入 `schemaStore.addWaitingUploadPatches` 队列，通过 WebSocket 飞向服务器，并最终被房间内的其他客户端通过 `useApplyNetworkPatches` 接收并静默合并。

---

#### 4. 性能黑魔法：选择性 CRDT 实例化 (Constant Node Pruning)
在将庞大的 JSON Schema 转换为 CRDT 模型时，如果为深层嵌套的每一个属性都创建 CRDT 节点（每个节点都需要维护逻辑时钟、Tombstone 等元数据），会导致灾难性的内存开销和极慢的初始化速度。

Zion 在 `@functorz/crdt-helper` 中引入了一个非常硬核的启发式剪枝引擎（`ALL_NODE_MATCH`）。它会在递归构建 CRDT 树时，精准识别出那些**不需要细粒度协同**或**总是被整体替换**的子树（例如 `componentFrame`、`tableMetadata`、`dataBinding` 等）。
...
这场“大换血”式的重构，彻底拔掉了压在 Zion 基础设施头上的 OOM 梦魇。

从中心化的 **Stateful Diff Server** 演进到 **Stateless CRDT Architecture**，我们不仅优化了服务器的内存占用，更为用户带来的是**真正的 Local-first (本地优先)** 编辑体验——网络再差、甚至断网离线，用户依然可以毫无延迟地疯狂拖拽组件，等网络恢复时，CRDT 会用抹平分歧。
